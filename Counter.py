...
def attempt_count(blob, blob_id, counting_line, counts,classes):
    '''
    Check if a blob has crossed a counting line.
    '''
    label = counting_line['label']
    if _has_crossed_counting_line(blob.bounding_box, counting_line['line']) and label not in blob.lines_crossed:

        if classes[blob.type] in counts[label]:
            counts[label][classes[blob.type]]=counts[label][classes[blob.type]]+1

        else:
            counts[label][classes[blob.type]] = 1
